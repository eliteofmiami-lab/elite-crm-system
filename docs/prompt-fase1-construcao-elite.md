...
**A13. Cupom de agendamento — $200 na alçada do Eugene.** Regra: o Eugene pode oferecer UM cupom de $200 por lead para fechar AGENDAMENTO/visita (hesitação na marcação, objeção de preço na hora de agendar, reativação de no-show/cold). Além de $200 = escalar ao Rafael. Implementação: (1) toggle "Offered $200 booking coupon" no Log call details + detecção na transcrição (`cupom_oferecido: {bool, contexto}`) — as duas fontes gravam em `coupons` no Supabase e em nota no GHL; (2) **briefing pré-venda exibe obrigatoriamente** cupom prometido ("$200 prometidos em call de [data]") — o Rafael nunca pode ser surpreendido na loja; (3) cards de follow-up/quote-sent mostram a linha de alçada; (4) métricas nos relatórios: cupons oferecidos/semana, taxa de conversão em appointment e em venda com vs. sem cupom, cupons expirados (lead não veio); (5) sem cap rígido por default — `coupon_monthly_cap` configurável se o Rafael quiser; padrão: visibilidade total em vez de trava. Nota de alinhamento: como a comissão do Eugene só paga em venda FECHADA, cupom em lead ruim não gera custo nem comissão — o incentivo se auto-regula; ainda assim, o relatório semanal destaca o uso para o Rafael calibrar.

**A14. Flags de ouro definidas + Appointments Board (spec 6.6).** (1) Depósito = Win: fluxo permanece manual do Rafael (marca Win + valor); implementar atalho `Comprou` no Appointments Board (campo de valor → write-through de Win + monetaryValue na opp). Receita do fechamento mensal = soma dos valores das opps Win no período. (2) Appointments Board na visão do Rafael: hoje/amanhã/últimos 7 dias, toques Confirmado/Showed/No-show/Comprou com os efeitos em cadeia do spec (status no GHL, flag de visita, resgate de no-show, Win/comissão, CAPI). `meta_events_map` em config com default: Showed = evento de qualificação, Comprou = Purchase com valor, No-show = não reporta — leads teste-interno nunca reportam. (3) Análise ganha `resolucao_da_call` (qualificou/avançou/desqualificou/pendente); "falou com o Rafael" NÃO pontua — desqualificação derruba score e pode mover para Lost com motivo. (4) Elegibilidade (complementa A7.4): contato com Win anterior que agenda de novo = SEM cards de venda; visível apenas no Board e no briefing (com histórico de serviços do Urable/GHL). Remover das REGRAS a proposta antiga de +10 para repetido/indicação.

**A15. Elite Rail — painel dentro do GHL (spec 6.7).** Ordem de execução: (1) HOJE: modo `?layout=rail` no painel (coluna única compacta: card de contexto + fila + aprovações; sem grid de KPIs) + gerar instruções para o Rafael criar o Custom Menu Link no GHL. (2) EM SEGUIDA: extensão Chrome MV3 "Elite Rail": coluna retrátil à direita em app.gohighlevel.com (iframe do modo rail), aba de borda quando fechada, atalho de teclado, leitura do contactId da URL → postMessage → card do lead em contexto no topo da coluna. Resolver frame-ancestors no CSP do painel (permitir app.gohighlevel.com) e auth em iframe (cookies SameSite=None; fallback: token de sessão). Entregar como load-unpacked com instruções de instalação para o Eugene; preparar empacotamento p/ Chrome Web Store unlisted (replicação em novos assistentes/shops). PRIORIDADE: somente após os itens da manhã (MANHA.md, G2, TOP50) — não compete com o go-live.

**A16. Síntese cronológica do lead — a correção arquitetural (REGRA ZERO).** A análise de call vira INSUMO; quem escreve score/next_action/card é a síntese por lead:
(1) **Pipeline:** a cada evento novo (call analisada, SMS in/out, quote, appointment, inbound perdida), montar a LINHA DO TEMPO compacta do lead (análises resumidas + mensagens + eventos, ordenados) → chamada de síntese (Sonnet, com caching do formato) → `estado_do_lead`: { situacao: ativo_venda | aguardando_decisao_cliente(data, evidência) | aguardando_evento_externo(qual, janela) | agendado(follow-up data) | callback_devido(URGENTE) | pos_venda | esfriou, momento_atual, intencao_atual (cada um com data da evidência mais recente), proxima_acao derivada DO ESTADO, narrativa_do_card (a história em 3 linhas) }.
(2) **Score sobre o estado atual**, não sobre a call mais antiga: componentes carregam data; evidência nova sobrescreve; "pediu espaço" reduz cadência (nurture leve configurável) sem zerar valor.
(3) **Detecção que faltou:** inbound perdida de CONTATO EXISTENTE (não só número novo) = callback_devido no topo da Camada 1 — caso Agie provou o buraco; verificar e corrigir a detecção.
(4) **GATE G2 v3 — critério de aceite com gabarito do Rafael:** reemitir o demo como DOSSIÊS dos mesmos 5 leads (linha do tempo completa → estado atual → o que seria gravado). O estado de cada um DEVE bater com a leitura do Rafael: Robert = aguardando_decisao_cliente desde 9/jun (nurture leve, fora da discagem ativa) · Shawn = agendado, só follow-up na data, nada mais · Adam = aguardando_evento_externo (Model YL, produção set/lançamento fim de ano → follow-up ~setembro) · Naomi = pos_venda, ZERO cards de venda · Agie = callback_devido URGENTE (ela retornou a ligação e não foi atendida). Se qualquer um divergir do gabarito, não emitir — investigar e corrigir. Incluem-se as 4 correções anteriores (merge de engajamento no payload, tipo_de_call, rascunhos em 1ª pessoa naturais, inferência de ano).
(5) **Fila e análise total:** a Onda 0/1 passa a produzir sínteses de lead (não só análises de call); custo controlado por linhas do tempo compactas + caching — reportar impacto no cost_log.

**A16.1 Pós-venda FORA do sistema (corrige A14/A16 no ponto).** `tipo_de_call = pos_venda_garantia` → o sistema NÃO executa NADA: sem card, sem task (nem de retorno), sem score de venda, sem mover stage, sem nice-to-talk, sem advice, sem entrada em board/briefing gerada pelo cérebro. Única ação: gravar a classificação `pos_venda` na análise (para excluir das métricas de venda) e permanecer inerte. Gestão de pós-venda é pessoal do Rafael, um a um. Reentrada na máquina de venda SOMENTE se o contato iniciar conversa de venda nova (novo veículo/serviço) — tratada como lead normal. Gabarito da Naomi no G2_DEMO_v3 atualizado: estado = pos_venda → o que seria gravado = NADA (apenas a classificação no registro).
